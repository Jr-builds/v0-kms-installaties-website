...
function resolveProjectImage(
  ...
  return imageSources[imageKey] ?? { src: getImage(imageKey).src, alt: getImage(imageKey).alt }
}

export default function ProjectenClient({ imageSources }: ProjectenClientProps) {
        ...
        <section className="bg-kms-light py-12 sm:py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {filtered.length === 0 ? (
              <div
                className={cn(
                  'project-grid-fade rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-16 text-center',
                  prefersReducedMotion && 'motion-disable',
                )}
              >
              ...
              >
                {filtered.map((project) => {
                  ...
                  const image = resolveProjectImage(imageKey, imageSources)
                      ...
                      >
                        ...
                        <div className="p-5">
                          ...
                          <span className="inline-flex items-center text-sm font-semibold text-kms-navy group-hover:underline">
                            ...
                            <span
                              aria-hidden="true"
                              className="ml-1 motion-safe:transition-transform motion-safe:group-hover:translate-x-0.5"
                            >
                              →
                            </span>
                          </span>
